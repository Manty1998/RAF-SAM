...
def calc_loss(outputs, low_res_label_batch, ce_loss, dice_loss, arg, dice_weight: float = 0.8):
    ...
    # 添加一致性损失
    consistency_loss = outputs.get('total_consistency_loss', 0.0) * 0.1   #     arg.consistency_frequency = 0.1

    loss = loss1 + loss2 + consistency_loss
    return loss, loss1, loss_ce1, loss_dice1, loss2, loss_ce2, loss_dice2, consistency_loss
...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_random_new_mean_up(args, model, snapshot_path, multimask_output, low_res):
    ...
    if args.AdamW:
        ...
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=b_lr, momentum=0.9,
                              weight_decay=0.0001)  # Even pass the model.parameters(), the `requires_grad=False` layers will not update
    writer = SummaryWriter(snapshot_path + '/log')
    iter_num = 0
    max_epoch = args.max_epochs  # 5000
    stop_epoch = args.stop_epoch
    max_iterations = args.max_epochs * len(trainloader)
    logging.info("{} iterations per epoch. {} max iterations ".format(len(trainloader), max_iterations))
    best_performance = 0.0
    iterator = tqdm(range(max_epoch), ncols=70)
    for epoch_num in iterator:  # 5000
        for i_batch, sampled_batch in enumerate(trainloader):  # 11
            ...
            supervised_loss1, loss_ce1, loss_dice1 = calc_loss_labeled(outputs1, label_batch, ce_loss, dice_loss,
                                                                       labeled_bs, args.dice_param)
            supervised_loss2, loss_ce2, loss_dice2 = calc_loss_labeled(outputs2, label_batch, ce_loss, dice_loss,
                                                                       labeled_bs, args.dice_param)
            loss1 = supervised_loss1 + supervised_loss2

            new_consistency_loss = 0.0
            # 添加 new_consistency_loss
            new_consistency_loss = outputs.get('new_total_consistency_loss', 0.0) * 0.1 # args.consistency_frequency
            loss1 += new_consistency_loss
            ...
            # the second round
            if iter_num < args.warm_iter:  # 5000
                ...
                supervised_round2_loss1, loss_round2_ce1, loss_round2_dice1 = calc_loss_labeled(outputs_round2_1,
                                                                                                label_batch, ce_loss,
                                                                                                dice_loss, labeled_bs,
                                                                                                args.dice_param)

                supervised_round2_loss1_r, loss_round2_ce1_r, loss_round2_dice1_r = calc_loss_labeled(
                    outputs_round2_1_r, label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)
            ...
            if iter_num < args.warm_iter:
                ...
                consistency_loss2 = 0.5 * (ce_loss(outputs_round2_2[labeled_bs:], pseudo_outputs1.long()) + dice_loss(
                    outputs_round2_2[labeled_bs:], pseudo_outputs1, softmax=True))

                consistency_loss1_r = 0.5 * (
                            ce_loss(outputs_round2_1_r[labeled_bs:], pseudo_outputs1.long()) + dice_loss(
                        outputs_round2_1_r[labeled_bs:], pseudo_outputs1, softmax=True))

                # print(f"supervised_round2_loss1 is {supervised_round2_loss1}, supervised_round2_loss1_r is {supervised_round2_loss1_r}, consistency_loss2 is {consistency_loss2}, consistency_loss1_r is {consistency_loss1_r}")
            ...
            if iter_num < args.warm_iter:
                ...
                supervised_round3_loss1, loss_round3_ce1, loss_round3_dice1 = calc_loss_labeled(outputs_round3_2,
                                                                                                label_batch, ce_loss,
                                                                                                dice_loss, labeled_bs,
                                                                                                args.dice_param)
                supervised_round3_loss1_r, loss_round3_ce1_r, loss_round3_dice1_r = calc_loss_labeled(
                    outputs_round3_2_r, label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)
            ...
            if iter_num < args.warm_iter:
                ...
                consistency_loss1 = 0.5 * (ce_loss(outputs_round3_1[labeled_bs:], pseudo_outputs2.long()) + dice_loss(
                    outputs_round3_1[labeled_bs:], pseudo_outputs2, softmax=True))
                consistency_loss2_r = 0.5 * (
                            ce_loss(outputs_round3_2_r[labeled_bs:], pseudo_outputs2.long()) + dice_loss(
                        outputs_round3_2_r[labeled_bs:], pseudo_outputs2, softmax=True))

                # print(f"supervised_round3_loss1 is {supervised_round3_loss1}, supervised_round3_loss1_r is {supervised_round3_loss1_r}, consistency_loss1 is {consistency_loss1}, consistency_loss2_r is {consistency_loss2_r}")
            ...
            writer.add_scalar('info/new_consistency_loss', new_consistency_loss, iter_num)
            ...
            if iter_num < args.warm_iter + 1:
                ...
                logging.info('iteration %d : loss : %f, loss1: %f, loss2: %f, loss3: %f' % (
                iter_num, loss.item(), loss1.item(), loss2.item(), loss3.item()))
