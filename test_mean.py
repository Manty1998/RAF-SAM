...
def inference(args, multimask_output, db_config, model, test_save_path=None):
    ...
    for case in tqdm(image_list):
        ...
        metric_i = test_single_volume_mean(image, label, model, classes=args.num_classes,
    ...
    # save:
    write_csv = "output/save_excel" + args.exp + "_test_mean.csv"
    save = pd.DataFrame({'RV-dice': first_list[:, 0], 'RV-hd95': first_list[:, 1], 'RV-asd': first_list[:, 2],
                         'RV-jc': first_list[:, 3], 'Myo-dice': second_list[:, 0], 'Myo-hd95': second_list[:, 1],
                         'Myo-asd': second_list[:, 2],
                         'Myo-jc': second_list[:, 3], 'LV-dice': third_list[:, 0], 'LV-hd95': third_list[:, 1],
                         'LV-asd': third_list[:, 2], 'LV-jc': third_list[:, 3]})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='pth/sam_vit_b_01ec64.pth',
                        help='Pretrained checkpoint')

    parser.add_argument('--lora_ckpt', type=str, default='best_model.pth', help='The checkpoint from LoRA')
